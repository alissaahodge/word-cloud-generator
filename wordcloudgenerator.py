...
def calculate_frequencies(file_contents):
    ...
    for content in file_contents1:
        ...
        for word in uninteresting_words:
            if word == content.lower():
                uninteresting = True
        if uninteresting is False:
            if content in frequency_dict:
                frequency_dict[content] += 1
            else:
                frequency_dict[content] = 1

    # wordcloud
    cloud = wordcloud.WordCloud()
    cloud.generate_from_frequencies(frequencies=frequency_dict)
